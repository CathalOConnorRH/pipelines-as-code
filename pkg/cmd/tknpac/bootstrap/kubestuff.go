...
func deleteSecret(ctx context.Context, run *params.Run, opts *bootstrapOpts) error {
	return run.Clients.Kube.Cluster(logicalcluster.Name{}).CoreV1().Secrets(opts.targetNamespace).Delete(ctx, secretName, metav1.DeleteOptions{})
}
...
func createPacSecret(ctx context.Context, run *params.Run, opts *bootstrapOpts, manifest *github.AppConfig) error {
	_, err := run.Clients.Kube.Cluster(logicalcluster.Name{}).CoreV1().Secrets(opts.targetNamespace).Create(ctx, &corev1.Secret{
